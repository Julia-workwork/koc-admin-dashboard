...
export const OPTIONS = {
  ...
  channel: ["INS", "Instagram", "TikTok", "YouTube", "Facebook"],
  priority: ["High", "Medium", "Low"],
  followUpStatus: ["Needed", "Done"],
};
...
export function normalizeKocRow(row, rowNumber) {
  ...
  const updateInput = clean(row["Update Input - Write Here"]);
  return {
    raw: row,
    rowNumber,
    no: clean(row["No."]),
    ...
    followUpStatus: clean(row["Follow-up Status"]),
    ...
    links: extractUrls(row.Resources, row.Notes, row["Extended Background"], row["Update Input - Write Here"]),
...
export function buildTodayBuckets(rows, now = new Date()) {
  ...
  const needFollowUp = rows.filter((row) => {
    if (row.followUpStatus === "Done") return false;
    if (row.followUpStatus === "Needed") return true;
